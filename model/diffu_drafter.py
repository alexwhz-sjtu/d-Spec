...
class DreamModel(DreamGenerationMixin, DreamPreTrainedModel):
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor, generation_config: DreamGenerationConfig,):
        ...
        for i in range(steps):
            ...
            # create sample candidate for every postion via entropy.
            top_tokens, top_probs, entropy = sample_candidates(
                mask_logits,
        ...
        draft_tokens = []            # 最终拼接的 token ID 列表
        retrieve_paths = []          # 每条路径：[id0, id1, id2]
        token_to_global_id = {}      # (pos, token) -> global_id
        global_id_to_token = {}      # global_id -> token
        children_map = defaultdict(list)  # parent_id -> [child_id_list]
        parent_map = {}              # child_id -> parent_id

        # 根节点（anchor token，比如上一步采样的 token）
        anchor_token=input_ids[:, -1:].item()  # 假设你有这个值
        root_id = 0
        token_to_global_id[('root', anchor_token)] = root_id
        global_id_to_token[root_id] = anchor_token
        draft_tokens.append(anchor_token)

        next_id = 1  # 下一个可用的 global ID

        prev_level_ids = [root_id]  # 上一层所有节点的 global_id（初始为根）

        for pos, cand in enumerate(candidates_list):
            current_level_ids = []
            k = len(cand['tokens'])

            # 当前层每个 candidate 都要连接到上一层的所有节点？
            # ❌ 不合理！应该是：位置 i 的 token 只能接在位置 i-1 的任意 token 后面
            # ✅ 正确语义：笛卡尔积扩展

            for token_idx in range(k):
                token = cand['tokens'][token_idx].item()

                # 为当前 token 分配一个 global ID
                global_id = next_id
                next_id += 1
                ...
                token_to_global_id[(pos, token)] = global_id
                global_id_to_token[global_id] = token
                current_level_ids.append(global_id)

                # 连接到上一层所有节点（即所有 prefix 都可以延伸）
                for parent_id in prev_level_ids:
                    children_map[parent_id].append(global_id)
                    parent_map[global_id] = parent_id

            prev_level_ids = current_level_ids  


        def dfs_path(node_id, path, leaf_paths, max_depth, children_map):
            if node_id not in children_map or len(children_map[node_id]) == 0:
                ...
                for child_id in children_map[node_id]:
                    ...
                    dfs_path(child_id, path, leaf_paths, max_depth, children_map)
                    path.pop()

        leaf_paths = []
        dfs_path(root_id, [root_id], leaf_paths, max_depth=100, children_map=children_map)

        # 转为 tensor
        max_len = max(len(p) for p in leaf_paths) if leaf_paths else 1
        retrieve_indices = torch.full((len(leaf_paths), max_len), fill_value=-1, dtype=torch.long)

        for i, path in enumerate(leaf_paths):
            retrieve_indices[i, :len(path)] = torch.tensor(path, dtype=torch.long)


        total_nodes = len(draft_tokens)  # 包括 root
        tree_mask = torch.eye(total_nodes, dtype=torch.float32)[None, None]  # (1,1,N,N)

        # create mask
        for i in range(total_nodes):
            ancestor = i
            while ancestor in parent_map:
                ancestor = parent_map[ancestor]
                tree_mask[0, 0, i, ancestor] = 1.0
            tree_mask[0, 0, i, i] = 1.0  # 自己可见

        # position_ids = depth in tree
        tree_position_ids = torch.zeros(total_nodes, dtype=torch.long)
        for i in range(total_nodes):
            depth = 0
            curr = i
        ...
        draft_tokens_tensor = torch.tensor(draft_tokens, dtype=torch.long)[None]  # (1, N)
        retrieve_indices = retrieve_indices.to(draft_tokens_tensor.device)
        tree_mask = tree_mask.to(draft_tokens_tensor.device)
        tree_position_ids = tree_position_ids.to(draft_tokens_tensor.device)
